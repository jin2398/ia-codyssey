...
class MissionComputer:  # MissionComputer라는 이름의 클래스(설계도)를 정의함
    ...
    def get_mission_computer_info(self):  # 운영체제, CPU, 메모리 등 시스템 정보를 출력하는 함수
        ...
        try:  # 오류 발생 시 프로그램이 멈추지 않도록 보호
            ...
            filtered_info = {
                key: value #조건을 통과한 key와 value를 그대로 새로운 딕셔너리에 저장
                for key, value in info.items() # info 딕셔너리의 모든 (키, 값)을 하나씩 꺼냄
                if self.settings.get(key, 1) == 1 # setting.txt에서 해당 key의 값을 확인 1인 경우애만 출력
            }

            print(json.dumps(filtered_info, indent=2))# 수집한 시스템 정보를 JSON 형식으로 보기 좋게 출력

            return filtered_info  # 수집한 정보를 딕셔너리로 반환 (다른 곳에서 활용 가능하도록)
